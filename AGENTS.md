# Agent Guidelines for Affiliate Next Orchestrator

## Build & Development Commands

- `pnpm dev` - Start all workspaces in development
- `pnpm build` - Build all workspaces
- `pnpm check-types` - Type checking across all workspaces
- `pnpm dev:web` - Run web app only (port 3001)
...
- **Imports**: Use `@/` alias for web app components/utils, prefer named imports
- **Components**: Use functional components with TypeScript interfaces, destructure props
- **Styling**: TailwindCSS with class-variance-authority (cva) for component variants
...
- Monorepo with pnpm workspaces: `apps/web` (Next.js) + `packages/backend` (Convex)
- Authentication via better-auth with Convex adapter
